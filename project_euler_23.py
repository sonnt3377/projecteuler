...
def find_divisor_sum(number):
    ...
    """
    sum = 1 # 1 is always added to the sum since 1 is the divisor of all numbers
...
def create_sum_abundant_list(number):
    ...
    For the list item at index = i, marked it as True if that index is the sum of two abundant numbers, and False otherwise
    ...
    for i in range(len(abundant_list)):
        for j in range(i, len(abundant_list)):
            ...
            sum = abundant_list[i] + abundant_list[j]
            if (sum <= number):
                list[sum - 1] = True
...
def find_sum_of_non_abundant_numbers(number):
    ...
    """
    sum = 0
    list = create_sum_abundant_list(number)
